...
<body class="login admin-login">
    <div class="bg">
        <div id="wrapper">
            <div class="logo"><img src="{$host}zb_system/image/admin/none.gif" title="{htmlspecialchars($blogname)}" alt="{htmlspecialchars($blogname)}" /></div>
            <div class="login">
                <form method="post" action="#">
                    <input type="hidden" name="csrfToken" value="{$zbp.GetCSRFToken('login','minute')}">
                    <dl>
                        <dt></dt>
                        <dd class="username"><label for="edtUserName">{$lang['msg']['username']}</label><input type="text" id="edtUserName" name="edtUserName" size="20" value="{php}echo GetVars('username', 'COOKIE');{/php}" tabindex="1" /></dd>

                        <dd class="password"><label for="edtPassWord">{$lang['msg']['password']}</label><input type="password" id="edtPassWord" name="edtPassWord" size="20" tabindex="2" /></dd>

                        {if $zbp->option['ZC_LOGIN_VERIFY_ENABLE']}
                        <dd class="validcode"><label for="edtValidcode">{$lang['msg']['validcode']}</label><input type="text" maxlength="{$zbp.option['ZC_VERIFYCODE_LENGTH']}" id="edtValidcode" name="verify" size="20" tabindex="10" />
                            <img src="{$host}zb_system/script/c_validcode.php?id=login&time=m" onClick="javascript:this.src='{$host}zb_system/script/c_validcode.php?id=login&time=m&tm='+Math.random();" alt="validcode" />
                        </dd>
                        {/if}
