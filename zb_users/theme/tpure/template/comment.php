<div class="cmtsitem">
	...
	<div class="cmtscon">
		<div class="cmtshead">
			<div class="cmtsname">{if $comment.Author.HomePage}<a href="{$comment.Author.HomePage}" rel="nofollow" target="_blank">{$comment.Author.StaticName}</a>{else}{$comment.Author.StaticName}{/if}{if $zbp->Config('tpure')->PostCMTIPON == '1' && tpure_ipLocation($comment.IP)} <em>IP:{tpure_ipLocation($comment.IP)}</em>{/if}</div>
			{if $zbp->Config('tpure')->PostCMTTIMEON}<div class="cmtsdate">{tpure_TimeAgo($comment->Time(),$zbp->Config('tpure')->PostTIMESTYLE)}</div>{/if}
		</div>
