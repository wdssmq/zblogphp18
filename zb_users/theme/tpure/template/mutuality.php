...
$relatenum = $zbp->Config('tpure')->PostRELATENUM ? $zbp->Config('tpure')->PostRELATENUM : 6;
$str = '';
if(empty($tagid)){
	$tagrd = 0;
}else{
			...
			if($zbp->Config('tpure')->PostRELATESTYLE == '0'){
				...
				$str .= '<div class="relateinfo">
						...
						if(tpure_isMobile()){$str .= '<a href="'.$related->Url.'">';}
							$str .= tpure_GetIntro($related);
						if(tpure_isMobile()){$str .= '</a>';}
						$str .= '</p>';
					$str .= '</div>
				</div>';
			}else{
				$str .= '<div class="relatelist"><a href="'.$related->Url.'" title="'.$related->Title.'">'.$related->Title.'</a><span class="posttime">'.tpure_TimeAgo($related->Time(), $zbp->Config('tpure')->PostTIMESTYLE).'</span></div>';
			}
		}
	}
{/php}
