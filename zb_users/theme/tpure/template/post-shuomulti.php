...
<div class="shuo item">
	...
	{if $zbp->Config('tpure')->PostSHUOTITLEON}<h2><a href="{$article.Url}"{if $zbp->Config('tpure')->PostBLANKSTYLE == 2} target="_blank"{/if}>{if $zbp->Config('tpure')->PostMEDIAICONSTYLE == '0'}{if $article.Metas.audio}<span class="zbaudioicon"></span>{/if}{if $article.Metas.video}<span class="video"></span>{/if}{/if}{$article.Title}{if $zbp->Config('tpure')->PostMEDIAICONSTYLE == '1'}{if $article.Metas.audio}<span class="zbaudioicon"></span>{/if}{if $article.Metas.video}<span class="video"></span>{/if}{/if}</a></h2>{/if}
	<div class="intro">
	...
	<div class="thumbs timgbox">
		{php}
			$rawThumbs = tpure_Thumb($article, '0', true);
			$allThumbs = array_filter($rawThumbs);
			if (!$allThumbs) {
				$def = tpure_Thumb($article);
				if ($def) { $allThumbs = [$def]; }
			}
		{/php}
		{if $allThumbs}
