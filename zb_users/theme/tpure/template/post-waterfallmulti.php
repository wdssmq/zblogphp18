...
<div class="item waterfall-item">
	...
	<div class="waterfallimg{if $article->Metas->video} v{/if}">
		<a href="{$article.Url}"{if $zbp->Config('tpure')->PostBLANKSTYLE == 2} target="_blank"{/if}>{if $type != 'category'}<span>{$article.Category.Name}</span>{/if}<img src="{tpure_Thumb($article,1)}" alt="{$article.Title}"></a>
	</div>
	{/if}
