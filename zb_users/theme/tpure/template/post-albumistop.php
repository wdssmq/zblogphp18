...
<div class="item">
	...
	<div class="albumimg{if $article->Metas->video} v{/if}">
		<a href="{$article.Url}"{if $zbp->Config('tpure')->PostBLANKSTYLE == 2} target="_blank"{/if}>{if $type != 'category'}<span>{$article.Category.Name}</span>{/if}<img src="{if tpure_Thumb($article,1)}{tpure_Thumb($article,1)}{else}{$host}zb_users/theme/{$theme}/style/images/dot.jpg{/if}" alt="{$article.Title}"><em class="istop">{$lang['tpure']['istop']}</em></a>
	</div>
	{/if}
	<div class="albumcon">
